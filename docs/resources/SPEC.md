...
**A storage location, not an access-control primitive.** The crate's Argon2id + AES-256-GCM sealing is the primary access control for anything stored here; the OS credential store is defence-in-depth only. The access boundary differs by platform: macOS Keychain applies a per-application ACL, but one gated by user consent — a same-user process triggers a prompt the user may answer "Always Allow", and the trusted-application designation rests on a code signature a same-user process can generally overwrite — while a Windows generic Credential Manager entry is DPAPI-protected under the logged-in user's key and readable by **any process running as that user** (a per-USER boundary) — and, since entries are written with `CRED_PERSIST_ENTERPRISE`, roaming with the user profile on a domain-joined host rather than staying on the machine that wrote it. Callers MUST NOT file plaintext secrets here; the backend is a byte-blob KV store and does not enforce that. The credential store is released by the login session, so a machine/system service MUST NOT use this backend. The normative statement is the root `SPEC.md` §10.5.
